...
// Create a bit set lookup table for data bus - wastes 1kbyte of RAM but speeds things up
// dramatically can then use e.g. GPIO.out_w1ts = set_mask(0xFF); to set data bus to 0xFF
...
#define GPIO_DIR_MASK                                                                              \
    ((1 << TFT_D0) | (1 << TFT_D1) | (1 << TFT_D2) | (1 << TFT_D3) | (1 << TFT_D4) |               \
     (1 << TFT_D5) | (1 << TFT_D6) | (1 << TFT_D7))
...
#define set_mask(C)                                                                                \
    xset_mask[C] // 63fps Sprite rendering test 33% faster, graphicstest only 1.8% faster than
...
/* Old macros
  ...
  // Write two address coordinates
  #define tft_Write_32C(C,D) TFT_WRITE_BITS((uint16_t)((D)<<8 | (D)>>8)<<16 | (uint16_t)((C)<<8 |
(C)>>8), 32)

  // Write same value twice
  #define tft_Write_32D(C) TFT_WRITE_BITS((uint16_t)((C)<<8 | (C)>>8)<<16 | (uint16_t)((C)<<8 |
(C)>>8), 32)
//*/
